...
def mapper(parameter, genome, tree):
    """
    ...
    if genome:
        ...
        if parameter.params['GENOME_OPERATIONS']:
            ...
            phenotype, genome, tree, nodes, invalid, depth, \
                used_codons = map_ind_from_genome(parameter, genome)

        else:
            # Build the tree using algorithm.mapper.map_tree_from_genome().
            phenotype, genome, tree, nodes, invalid, depth, \
                used_codons = map_tree_from_genome(parameter, genome)
        ...
        nodes = tree.get_tree_info(parameter.params['BNF_GRAMMAR'].non_terminals.keys(),
                                   [], [])
        used_codons, phenotype = len(genome), "".join(output)
    
    if parameter.params['BNF_GRAMMAR'].python_mode and not invalid:
...
def map_ind_from_genome(parameter, genome):
    """
    ...
    max_tree_depth, max_wraps = parameter.params['MAX_TREE_DEPTH'], parameter.params['MAX_WRAPS']
    bnf_grammar = parameter.params['BNF_GRAMMAR']
...
def map_tree_from_genome(parameter, genome):
    """
    ...
    tree = Tree(parameter, str(parameter.params['BNF_GRAMMAR'].start_rule["symbol"]), None)

    # Map tree from the given genome
    output, used_codons, nodes, depth, max_depth, invalid = \
        genome_tree_map(parameter, tree, genome, [], 0, 0, 0, 0)
...
def genome_tree_map(parameter, tree, genome, output, index, depth, max_depth, nodes,
                    invalid=False):
    """
    ...
    """
    if not invalid and index < len(genome) * (parameter.params['MAX_WRAPS'] + 1):
        ...
        if parameter.params['MAX_TREE_DEPTH'] and (max_depth > parameter.params['MAX_TREE_DEPTH']):
            # We have breached our maximum tree depth limit.
            invalid = True
        ...
        productions = parameter.params['BNF_GRAMMAR'].rules[tree.root]['choices']
        no_choices = parameter.params['BNF_GRAMMAR'].rules[tree.root]['no_choices']
        ...
        for symbol in chosen_prod['choice']:
            ...
            if symbol["type"] == "T":
                ...
                output, index, nodes, d, max_depth, invalid = \
                    genome_tree_map(parameter, tree.children[-1], genome, output,
    ...
    NT_kids = [kid for kid in tree.children if kid.root in
               parameter.params['BNF_GRAMMAR'].non_terminals]
